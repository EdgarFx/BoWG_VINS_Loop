...
#include "ThirdParty/BoWG/Parameters.h"
...
int main(int argc, char **argv)
{
    ...
    if(argc != 3)
    {
        printf("please intput: rosrun loop_fusion loop_fusion_node [vins config file] [bowg config file]\n"
               "for example: rosrun loop_fusion loop_fusion_node "
               "/home/shawn_fei/catkin_ws/src/VINS-Fusion/config/euroc/euroc_stereo_imu_config.yaml \n"
               "/home/shawn_fei/catkin_ws/src/VINS-Fusion/config/euroc/euroc_bowg_config.yaml \n");
        return 0;
    }
    
    string bowg_config_path = argv[2];
    printf("bowg_config_file: %s\n", argv[2]);
    if (!Parameters::readParameters(bowg_config_path)) {
        std::cerr << "Failed to read parameters from: " << bowg_config_path << std::endl;
        return -1;
    }
