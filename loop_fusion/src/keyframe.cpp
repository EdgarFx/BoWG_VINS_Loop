...
// add by BoWG
void KeyFrame::computeMultiBRIEFPoint()
{
	// use multi-scale fast
	std::vector<cv::Mat> gray_pyramid; 
	gray_pyramid.push_back(image);
	...
	for (int i = 0; i < MS_LEVELS; i++)
	{
		...
		cv::FAST(gray_pyramid[i], tmp_keypts, fast_th, true);
...
bool KeyFrame::findConnection(KeyFrame* old_kf)
{
	...
	TicToc t_match;
		...
		if (DEBUG_IMAGE)    
	    {
	        ...
	        ostringstream path;
	        path << "/home/shawn/bowg_loop_debug/bowg_loop_image/"
	                << index << "-"
