 ...
 * Modified by: Xiang (Shawn) Fei
 *******************************************************/

#include "pose_graph.h"
#include "ThirdParty/BoWG/GeometicUtils.h"
...
void PoseGraph::addKeyFrame(KeyFrame* cur_kf, bool flag_detect_loop)
{
    ...
    if (flag_detect_loop)
    {
        ...
        db_wg.cur_image_id += 1; // add by BoWG
    }
    else
    {
...
{
    m_image_descriptors.push_back(keyframe->brief_descriptors);
    m_image_keys.push_back(keyframe->keypoints);
    map_kpts[frame_index] = keyframe->keypoints;
    map_des[frame_index] = keyframe->brief_descriptors;
    ...
    voc->transform(keyframe->brief_descriptors, cur_bow_vec);
    ...
    if (USE_WG) {
        ...
        {
            cv::KeyPoint tmp_kp1 = keyframe->keypoints[i];
            for (int j = i + 1; j < (int)keyframe->keypoints.size(); j++)
            {
                cv::KeyPoint tmp_kp2 = keyframe->keypoints[j];
                float distance = cv::norm(tmp_kp1.pt - tmp_kp2.pt);
                if (distance < tmp_kp1.size)
                {
                    //detect multi-scale feature and word pair
                    DBoW2::WordId wid1 = voc->transform(keyframe->brief_descriptors[i]);
                    BoWG::WordGroupID wg_id = db_wg.queryWordGroup(wid1);
                    kf_wgs.push_back(wg_id);
                }
                if (distance < tmp_kp2.size)
                {
                    DBoW2::WordId wid2 = voc->transform(keyframe->brief_descriptors[j]);
    ...
    if (GEOM_DI) {
        voc->transform(keyframe->brief_descriptors, cur_bow_vec, featvec, DI_LEVEL);
    }

    if (!(frame_index > DISLOCAL)) {
        if (GEOM_DI)
    ...
    // db.query(keyframe->brief_descriptors, ret, 3, frame_index - DISLOCAL);
    ...
    // need to first sort ret in ascending ID
        ...
        if (db_wg.matchGrouping(ret, groupMatches, SIMILARITY_TH, MAX_INTRAISLAND_GAP)) {
            ...
            m_fsolver.setImageSize(keyframe->image.cols, keyframe->image.rows);
                ...
                if (!USE_GEOM) {
                    ...
                    if (GEOM_DI) {
                        ...
                        if (isGeometricallyConsistent_DI<DVision::BRIEF::bitset, FBrief>(m_fsolver,db,m_image_descriptors,m_image_keys,best_entry_id,
                                keyframe->keypoints,keyframe->brief_descriptors,featvec,
                                MIN_FPOINTS,MAX_REPROJECTION_ERROR,RANSAC_PROBABILITY,MAX_RANSAC_ITERATIONS,MAX_NEIGHBOR_RATIO)){
                            find_loop = true;
                            final_loop_index = loop_ID;
                        }
                    }
                        ...
                        if (isGeometricallyConsistent_Exhaustive(map_des[frame_index],map_kpts[frame_index],map_des[loop_ID],map_kpts[loop_ID],inlierMatches,MIN_FPOINTS)){
                            find_loop = true;
                            final_loop_index = loop_ID;
                        }
                    }
        ...
        if (find_loop && frame_index > DISLOCAL)
        {
...
*/
    if (find_loop && frame_index > DISLOCAL)
    {
        int min_index = frame_index;
        for (unsigned int i = 0; i < ret.size(); i++)
        {
            if (ret[i].Id < min_index && ret[i].Score > SIMILARITY_TH)
                min_index = ret[i].Id;
        }
        return min_index;
    }
